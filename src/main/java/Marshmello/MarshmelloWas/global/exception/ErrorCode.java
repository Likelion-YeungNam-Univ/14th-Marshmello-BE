...
public enum ErrorCode {
    ...
    IMAGE_NOT_FOUND(HttpStatus.NOT_FOUND, "이미지를 찾을 수 없습니다.", false),
    CHECK_IN_ALREADY_EXISTS(HttpStatus.CONFLICT, "오늘 체크인은 이미 등록되었습니다.", false),
    IMAGE_ALREADY_USED(HttpStatus.CONFLICT, "이미 사용된 이미지입니다.", false),
    DUPLICATE_BODY_REGION(HttpStatus.BAD_REQUEST, "신체 부위가 중복되었습니다.", false),
    IMAGE_ANALYSIS_REQUIRED(HttpStatus.UNPROCESSABLE_ENTITY, "분석이 완료된 이미지가 필요합니다.", false),
    ...
    REPORT_ALREADY_EXISTS(HttpStatus.CONFLICT, "해당 월의 보고서가 이미 존재합니다.", false),
    ...
    REPORT_SOURCE_ERROR(HttpStatus.UNPROCESSABLE_ENTITY, "보고서에 사용할 분석 데이터가 부족합니다.", false),
